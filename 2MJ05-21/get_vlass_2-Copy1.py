...
def get_filename(co, tilename=None):
    ...
    res = requests.get('{0}/quicklook/VLASS1.2/{1}/'
...
def get_baseurl(co):
    ...
    base_url = '{0}/quicklook/VLASS1.2/{1}/{2}'.format(archive_url, tilename, file0)
    return base_url
...
def get_fitsname(co):
    ...
    file0 = get_filename(co, tilename=tilename)
    base_url = '{0}/quicklook/VLASS1.2/{1}/{2}'.format(archive_url, tilename, file0)
    file_url = 'VLASS1.2/{0}/{1}'.format(tilename, file0)
    res = requests.get(base_url)

    fitsfile = list(filter(lambda x: file0 in x and 'tt0.subim.fits' in x,
                           res.content.decode().split('\n')))[0].split('\"')[1]

    return '{0}/{1}'.format(base_url, fitsfile), str(fitsfile)
...
def get_fits(co, outname=None):
    ...
    fitsfile, filename = get_fitsname(co)
    if outname is None:
